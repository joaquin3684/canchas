...
object AdministracionVentaRepository extends Estados{
  ...
  def ventasPresentables(implicit obs: Seq[String]) : Future[Seq[(Venta, String, String, DateTime)]] = {
    ...
    val query = for{
      e <- estados.filter( x => x.estado === VISITA_CONFIRMADA && !(x.idVenta in estados.filter(x => x.estado === PRESENTADA || x.estado.like("%Rech%")).map(_.idVenta)))
      v <- ventas.filter(x => x.id === e.idVenta && x.idObraSocial.inSetBind(obs) && !x.empresa.isEmpty && !x.cuit.isEmpty && !x.tresPorciento.isEmpty)
    ...
    val query2 = for {
      e <- estados.filter( x => x.estado === VALIDADO && !(x.idVenta in estados.filter(x => x.estado === PRESENTADA || x.estado.like("%Rech%")).map(_.idVenta)))
      v <- ventas.filter(x => x.id === e.idVenta && x.idObraSocial.inSetBind(obs) && !x.empresa.isEmpty && !x.cuit.isEmpty && !x.tresPorciento.isEmpty)
  ...
  def ventasPresentadas(implicit obs: Seq[String]) : Future[Seq[(Venta, DateTime)]] = {
    val query = for {
      e <- estados.filter( x => x.estado === PRESENTADA && !(x.idVenta in estados.filter(x => x.estado === PAGADA || x.estado.like("%Rech%")).map(_.idVenta)))
      v <- ventas.filter(x => x.id === e.idVenta && x.idObraSocial.inSetBind(obs))
    } yield (v, e.fecha)
